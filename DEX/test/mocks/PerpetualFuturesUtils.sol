...
library PerpetualFuturesUtils {
    ...
    function collectMarginFees(DataTypes.ReserveData storage self, address _token, uint256 _sizeDelta, uint256 _size, uint256 _entryFundingRate, bool _isLong) internal returns (uint256) {
        uint256 feeUsd = getPositionFee(_sizeDelta);

        // 포지션 총 크기 * fundingRate
        uint256 fundingFee = getFundingFee(self, _token, _size, _entryFundingRate, _isLong);
    ...
    function getFundingFee(DataTypes.ReserveData storage self,address _token, uint256 _size, uint256 _entryFundingRate, bool _isLong) internal view returns (uint256) {
        if (_size == 0) { return 0; }
        // 오랫동안 포지션을 유지함에 따라 생기는 funding rate를 계산하여 fee에 적용
        uint256 fundingRate = self.cumulativeFundingRates[_token][_isLong] - _entryFundingRate;
    ...
    function validateLiquidation(DataTypes.ReserveData storage self, address _account, address _token, bool _isLong, bool _raise) internal view returns (uint256, uint256) {
        ...
        uint256 marginFees = getFundingFee(self, _token, position.size, position.entryFundingRate, _isLong);
        ...
        if (remainingCollateral * Constants.MAX_LEVERAGE < position.size * Constants.BASIS_POINT) {
            // 엄격 모드일 경우 예외처리
            if (_raise) { revert("PerpetualFuturesUtils: maxLeverage exceeded"); }
            // 아니라면 청산 가능 true, 남은 담보의 크기 리턴
            return (2, marginFees);
        }
    ...
    function reduceCollateral(DataTypes.ReserveData storage self, address _account, address _token, uint256 _collateralDelta, uint256 _sizeDelta, bool _isLong) internal returns (uint256, uint256) {
        ...
        params.fee = collectMarginFees(self, _token, _sizeDelta, position.size, position.entryFundingRate, _isLong);
    ...
    function increasePoolAmount(DataTypes.ReserveData storage self, address _token, uint256 _amount, bool _isLong) internal {
        self.tokenReserve[_token] = self.tokenReserve[_token] + _amount;
        uint256 balance = IERC20(_token).balanceOf(address(this));
        require(self.tokenReserve[_token] <= balance, "IncreasePoolAmount: pool amount is bigger than balance");
    ...
    function decreasePoolAmount(DataTypes.ReserveData storage self, address _token, uint256 _amount, bool _isLong) internal {
        self.tokenReserve[_token] = self.tokenReserve[_token] - _amount;
        require(self.tokenPositionData.reservedAmounts[_token][_isLong] <= self.tokenReserve[_token], "DecreasePoolAmount: reserve amount is bigger than pool amount");
    ...
    function increaseReservedAmount(DataTypes.ReserveData storage self, address _token, uint256 _amount, bool _isLong) internal {
        ...
        require(self.tokenPositionData.reservedAmounts[_token][_isLong] <= self.tokenReserve[_token], "IncreaseReservedAmount: reserve amount is bigger than pool amount");
