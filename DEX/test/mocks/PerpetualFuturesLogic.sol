...
library PerpetualFuturesLogic {
    ...
    function increasePosition(DataTypes.ReserveData storage self, DataTypes.PositionAdjustmentParams memory params) internal {
        self.updateCumulativeFundingRate(params.token, params.isLong);
        ...
        // 레버리지 수수료 계산
        uint256 fee = self.collectMarginFees(params.token, params.sizeDelta, position.size, position.entryFundingRate, params.isLong);
        ...
        require(position.collateral >= fee, "IncreasePosition: collateral can't afford to pay fee");

        // 감당 가능하다면 수수료만큼 포지션 담보에서 빼기
        position.collateral -= fee;
        // 포지션 시작 수수료 최신화 
        position.entryFundingRate = self.cumulativeFundingRates[params.token][params.isLong];
        // 포지션의 크기 최신화
