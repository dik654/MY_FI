...
library FlashLoanLogic {
    ...
    function executeFlashLoan(DataTypes.ReserveData storage self, address _token, uint256 _amount, address _to, address _contract, bytes memory _data) internal {
        ...
        // 컨트랙트인지 검증
        require(_to.code.length != 0, "ExecuteFlashLoan: Caller is not contract");
        ...
        // 사용자 로직 실행
        require(receiverContract.executeOperation(_token, _amount, fee, _contract, _data), "ExecuteFlashLoan: fail to execute operation");
        // 수수료를 더해서 상환
